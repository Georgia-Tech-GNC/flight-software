...
uint8_t lib_packet_encode(uint8_t message_id, RocketState *rocket_state, uint8_t *payload_buf, size_t payload_buf_size, size_t *bytes_written) {
    ...
    assert(msg_size > 0, "Encode valid packet");
    assert(msg_size <= payload_buf_size, "Encode packet size less than buffer size");
    ...
    switch(message_id) {
        ...
        case SERVODEFLECTIONS_MSG_ID:
            ...
            break;
        default:
            assert(FALSE, "Encode valid packet");
            break;
    }
...
uint8_t lib_csv_encode(RocketState *rocket_state, uint8_t *csv_line, size_t csv_line_size, size_t *bytes_written) {
    ...
    size_t bytes_copied = strncpy(csv_line, line, csv_line_size);

    if (!assert(bytes_copied <= csv_line_size, "CSV line size less than buffer size")) {
        return FAILURE;
    }
