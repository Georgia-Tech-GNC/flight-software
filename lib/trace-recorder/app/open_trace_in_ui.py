...
class HttpHandler(http.server.SimpleHTTPRequestHandler):
  ...
  def end_headers(self):
    self.send_header('Access-Control-Allow-Origin', self.server.allow_origin) # type: ignore
    self.send_header('Cache-Control', 'no-cache')
    super().end_headers()

  def do_GET(self):
    if self.path != '/' + self.server.expected_fname: # type: ignore
      self.send_error(404, 'File not found')
      return

    self.server.fname_get_completed = True # type: ignore
    super().do_GET()
...
def open_trace(path, open_browser, origin):
  ...
  with socketserver.TCPServer(('127.0.0.1', PORT), HttpHandler) as httpd:
    ...
    httpd.expected_fname = fname # type: ignore
    httpd.fname_get_completed = None # type: ignore

    # In case the user's `--origin` has a path component, strip it out
    def strip_url_path(url):
      url_parsed = urllib.parse.urlparse(url) # type: ignore
      return urllib.parse.urlunparse( # type: ignore
          url_parsed._replace(path='', query='', fragment='', params=''))

    httpd.allow_origin = strip_url_path(origin) # type: ignore
    while httpd.fname_get_completed is None: # type: ignore
