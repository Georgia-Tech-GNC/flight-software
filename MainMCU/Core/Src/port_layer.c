...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t size) {
    ...
    if (huart->Instance == telemetry_uart.Instance) {
        ...
        for (int i = 0; i < size; i++) {
            packet_buffer_size = process_incoming_byte(telemetry_uart_rx_buf[i], packet_buffer, packet_buffer_size);
            ...
            if (packet_buffer_size < 0) {
                ...
                if (msg_id == COMMAND_MSG_ID) {
                    ...
                    if (cmd.command_id < 10 && cmd.command_counter != last_cmd_counter[cmd.command_id]) {
                        ...
                        xStreamBufferSendFromISR(g_telemetry_command_stream_buffer.handle, &cmd.command_id, 1, &xHigherPriorityTaskWoken);
